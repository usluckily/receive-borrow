<template>
  <transition name="page">
    <div class="all">
      ...
      <f_btn>
        <router-link to="/receive/apply/" class="fbtn" tag="div" >{{ localBtnText }}</router-link>
      </f_btn>
    ...
    export default({
      ...
      data(){
        return {
          ...
          contentMargin:true,
          obj:config.getBase(this.$route.name)
        }
      },
      created(){
        let vm = this
...
//          showAll:obj.showall,
        ...
        if(vm.obj.status){
          pObj.status = vm.obj.status
        }

        vm.$root.eventHub.$on('slide',function(d){
          if(vm.obj.nav[d].id == 'frMe'){
            pObj.way = vm.obj.nav[d].id
            delete pObj.status
          }else{
            pObj.status = vm.obj.nav[d].id//获取status
          }
